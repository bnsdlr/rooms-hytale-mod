...
public class OtherValidators {
    ...
    public static final IncludesValidator<String> BLOCK_GROUPS;
    ...
    static {
        ...
        BLOCK_CUSTOM_MODEL = new IncludesValidator<>(customModels, "Couldn't match any existing custom models used for a block.", String::isBlank);
        ...
        BLOCK_HITBOX_TYPE = new IncludesValidator<>(hitboxTypes, "Couldn't match any existing hitbox types used for a block.", String::isBlank);

        Set<String> blockGroups = BlockType.getAssetMap().getAssetMap().values().stream().map(BlockType::getGroup).collect(Collectors.toSet());
        BLOCK_GROUPS = new IncludesValidator<>(blockGroups, "String doesn't match any existing block group.", String::isBlank);
    }
}
